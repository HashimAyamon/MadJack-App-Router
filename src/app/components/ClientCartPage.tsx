  ...
  if (cart.length === 0) {
    return (
      <div className="text-center py-5">
        ...
        <h2 className="fs-3 fw-bold pt-5 mb-3 text-primary">Your cart is empty</h2>
        <p className="text-muted">Start shopping to add items to your cart!</p>
        <Link href="/collections">
          <button className="btn btn-primary mt-3 shadow-sm">Buy Products</button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="container my-5 pt-5 text-black">
      <div className="d-flex align-items-center justify-content-between mb-4">
        ...
        <button className="btn btn-danger shadow-sm" onClick={clearCart}>
          Clear Cart
        </button>
      </div>
      ...
      <div className="bg-white rounded shadow-lg p-4">
        ...
        <div className="border-top pt-3">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <span className="fs-5 fw-semibold text-primary">Total:</span>
            <span className="fs-4 fw-bold text-success">${getCartTotal().toFixed(2)}</span>
          </div>
          <button className="btn btn-primary w-100 shadow-sm">Proceed to Checkout</button>
        </div>
      </div>
    </div>
  );
